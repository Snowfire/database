...
namespace Snowfire\Database;
use Snowfire\Database\Query\Helper;
use Snowfire\DatabaseInterface;
...
class Model
{
	...
	*/
	...
	public static function database(DatabaseInterface &$database)
	{
	...
	private function _create_many_to_many($id, $foreign_ids, self $foreign_model)
	{
	...
	private function _delete_one_to_one($rows, self $foreign_model, array $foreign)
	{
		$foreign_singular = isset($foreign['singular']) ? $foreign['singular'] : $foreign_model->singular();
		$ids = array();
	...
	private function _delete_many_to_many($rows, self $foreign_model)
	{
		$ids = array();
	...
	*/
	public function many($p = array())
	{
		...
		if (isset($p['conditions']) && $p['conditions']) {
			...
			} else if (is_array($p['conditions']) && Helper::is_associative($p['conditions'])) {
				$q->where($p['conditions']);
			} else {
	...
	* passports: id
	*/
	private function _get_one_to_one($single_row, $rows, self $model, array $foreign)
	{
		$called_class = get_called_class();
		$ids = array();
		$index = array();
	...
	* passports: id
	*/
	private function _get_many_to_one($single_row, $rows, self $model, array $foreign)
	{
		$called_class = get_called_class();
		$ids = array();
		$index = array();
	...
	*/
	private function _get_one_to_many($single_row, $rows, self $foreign_model)
	{
		$called_class = get_called_class();
	...
	*/
	...
	{
		return new Query($this->_database);
	}
